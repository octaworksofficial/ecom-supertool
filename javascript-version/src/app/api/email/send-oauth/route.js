...
export async function POST(request) {
  console.log('🚨 OAuth EMAIL API ÇAĞRILDI!')
...
return NextResponse.json(
    ...
    console.log('📧 Starting OAuth email send:', {
    ...
    for (let i = 0; i < emails.length; i++) {
      ...
      try {
        ...
        try {
          savedEmail = await prisma.emailSend.create({
            data: {
          ...
          console.log('✅ Email saved to database SUCCESS:', {
            id: savedEmail.id,
    ...
    console.log('🎯 OAuth email campaign completed:', {
      campaignId: campaignId,
...
return NextResponse.json(
      { error: 'Email gönderimi başarısız', details: error.message },
      { status: 500 }
    )
  }
}
...
function addTrackingToLinks(content, trackingId, baseUrl) {
  ...
  return content.replace(
    ...
    (match, attributes, originalUrl) => {
      const trackingUrl = `${baseUrl}/api/email/track?id=${trackingId}&url=${encodeURIComponent(originalUrl)}`
...
function encodeEmailHeader(text) {
  // Email header'ı encode et (UTF-8 karakterler için)
